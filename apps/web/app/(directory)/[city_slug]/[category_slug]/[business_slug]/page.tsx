...
export default async function BusinessDetailPage({ params }: PageProps) {
  ...
  const reviews = await db
    ...
    .from(schema.reviews)
    .where(eq(schema.reviews.businessId, business.id))
  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-4xl px-6 py-10">
        ...
        <div>
          <h2 className="text-xl font-bold sm:text-2xl">Community Reviews</h2>
          {reviews.length > 0 ? (
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <svg
                          key={star}
                          className={`h-4 w-4 ${star <= review.rating ? 'fill-amber-400 text-amber-400' : 'fill-none text-muted-foreground/30'}`}
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          strokeWidth={1.5}
                        >
